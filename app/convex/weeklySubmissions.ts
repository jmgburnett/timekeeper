...
export const submitWeek = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing?.status === "locked") {
      throw new Error("This week is locked. Cannot submit.");
    }
...
export const lockWeek = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Also create locked entries for participants who never submitted
    const participants = (
    ...
    for (const p of participants) {
      if (!submittedIds.has(p.slackUserId)) {
        await ctx.db.insert("weekly_submissions", {
